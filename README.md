...
Call `allowModifiers(true)` on a `KeybindOption` to accept Ctrl/Shift/Alt/Super chords. While capturing input, Escape selects the unbound state instead of closing the parent screen. The default keybind shipped by Mod Deck is unbound.
...
Mods can also register named presets that apply several typed draft values without saving them. Targets use both category and option IDs, so repeated option names in different categories remain unambiguous:
...
Unknown targets, duplicate preset IDs, non-persistent entries, incompatible types, and invalid values are rejected with an exception. Registered metadata is available through `presets()` and `preset(id)` for custom preset pickers or other integrations.
...
This is suitable for a mod's own settings button, Mod Menu, another settings hub, or a client command. Unknown mod IDs and routes fail immediately instead of opening an empty screen.
...
Bundled font and icon licenses are documented in [THIRD_PARTY_NOTICES.md](THIRD_PARTY_NOTICES.md) and included in the built jar under `META-INF/licenses/`.
...
The Cloth Config v26.2 feature audit and current parity decisions are documented in [docs/CLOTH_CONFIG_PARITY.md](docs/CLOTH_CONFIG_PARITY.md).
